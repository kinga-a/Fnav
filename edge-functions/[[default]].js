 ...
 * 
 * Project Structure:
 * /edge-functions/[[default]].js  - API catch-all route
 * /index.html                     - Single Page Application (SPA)
 * /edgeone.json                   - Project config
 * 
 * Environment Variables (set in EdgeOne Pages console):
 * - AUTH_PASSWORD: Password for accessing private bookmarks
 * - AUTH_USERNAME: Optional username (default: admin)
 * - KV_BINDING:    KV namespace binding variable name (default: BOOKMARK_KV)
 * 
 * KV Storage Schema:
 * - bookmarks:{id}    -> JSON bookmark object
...
<div id="app">
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <div class="nav-section">
      ...
      <div class="add-category-btn" onclick="app.showAddCategory()">
        <span>+</span> 新建分类
      </div>
    </div>
...
class BookmarkApp {
  ...
  async loadData() {
    try {
      ...
      if (categoriesRes.ok) {
        ...
        this.categories = data.categories || ['未分类'];
      }
    } catch (e) {
  ...
  getFilteredBookmarks() {
    ...
    } else if (this.currentView === 'private') {
      filtered = filtered.filter(b => b.isPrivate);
    } else if (this.categories.includes(this.currentView)) {
      filtered = filtered.filter(b => b.category === this.currentView);
    }
  ...
  createBookmarkCard(b) {
    ...
    return `
      <div class="bookmark-card ${b.pinned ? 'pinned' : ''} ${b.isPrivate ? 'private' : ''} ${isLocked ? 'private-locked' : ''}" data-id="${b.id}">
        <div class="card-header">
          <h3 class="card-title">${this.escapeHtml(b.title)}</h3>
          <div class="card-badges">
            ${b.pinned ? '<span class="badge badge-pinned">📌 置顶</span>' : ''}
            ${b.isPrivate ? '<span class="badge badge-private">🔒 私有</span>' : ''}
            <span class="badge badge-category" style="border-color: ${categoryColor}33; color: ${categoryColor}">${this.escapeHtml(b.category || '未分类')}</span>
          </div>
        </div>
        <p class="card-desc">${this.escapeHtml(b.description || '暂无描述')}</p>
        <div class="card-footer">
          ${isLocked 
            ? `<span class="card-link" style="color: var(--text-muted); cursor: not-allowed;">🔒 私有内容</span>`
            : `<a href="${this.escapeHtml(b.url)}" target="_blank" rel="noopener" class="card-link">🔗 ${this.truncateUrl(b.url)}</a>`
          }
          <div class="card-actions">
            <button onclick="app.editBookmark('${b.id}')" title="编辑">✏️</button>
            <button class="delete" onclick="app.deleteBookmark('${b.id}')" title="删除">🗑️</button>
          </div>
        </div>
      </div>
    `;
  ...
  renderCategories() {
    ...
    list.innerHTML = this.categories.map(cat => {
      ...
      return `
        <div class="category-item ${isActive ? 'active' : ''}" onclick="app.setView('${this.escapeHtml(cat)}')">
          <span class="cat-color" style="background: ${color}"></span>
          <span>${this.escapeHtml(cat)}</span>
          <span style="margin-left: auto; font-size: 0.75rem; opacity: 0.6;">${counts[cat] || 0}</span>
        </div>
      `;
    }).join('');
  ...
  async confirmDelete() {
    ...
    try {
      const res = await fetch(API_BASE + '/bookmarks/' + this.deleteTarget, {
        method: 'DELETE',
  ...
  async doAuth() {
    ...
    try {
      ...
      document.getElementById('authError').textContent = '验证失败';
    }
  }
